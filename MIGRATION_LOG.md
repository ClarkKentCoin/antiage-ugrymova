...
### Step 2.4 — Stabilize Test Mode (nocache + resolved identity diagnostics)

**Date/Time:** 2026-02-05 (UTC)

**Goal:** Eliminate caching effects in Test Mode (?test=1) and expose exactly what the server resolved (user_id + tenant) to diagnose "no subscription" issues that appear intermittently.

**Risk Level:** Low (additive diagnostics only, no breaking changes)
...
| File | Description |
|------|-------------|
| `supabase/functions/get-subscriber-status/index.ts` | Added `test_mode` parameter to skip initData validation for test flows; added `identity_source`, `resolved_user_id` to `_debug` response; bumped FUNCTION_VERSION to "2026-02-05_01:step2"; error responses now include `_debug` for visibility |
| `src/hooks/useSubscribers.tsx` | `useSubscriber` now accepts `testMode` parameter; adds `nocache: Date.now()` and `test_mode: true` to request body for test mode; query key includes timestamp for test mode to prevent stale cache |
| `src/components/telegram/MiniAppBuildBadge.tsx` | Extended `ServerDebugInfo` to include `_debug` with identity fields; added display of `src` (identity source), `uid` (resolved user id), `tn` (tenant); added `subscriberStatus` prop to show status |
| `src/pages/TelegramApp.tsx` | `useSubscriber` for test mode now passes `testMode: true`; `testDebugInfo` extracted from response with `_debug`; `MiniAppBuildBadge` receives test debug info and subscriber status |
...
**Lovable Rollback:**
- [ ] Revert `supabase/functions/get-subscriber-status/index.ts` (remove test_mode handling and identity_source diagnostics)
- [ ] Revert `src/hooks/useSubscribers.tsx` (remove testMode parameter and nocache logic)
- [ ] Revert `src/components/telegram/MiniAppBuildBadge.tsx` (remove identity diagnostics display)
- [ ] Revert `src/pages/TelegramApp.tsx` (remove testDebugInfo and testMode flag)
...
- [ ] In browser with ?test=1, entering a known Telegram User ID consistently returns correct status
- [ ] Debug badge shows `src: test`, `uid: <telegram_user_id>`, `tn: <tenant>`
- [ ] Debug badge shows `status: active/grace_period/expired/none` as appropriate
- [ ] Refreshing the page with same user ID shows consistent results (no random "empty" results)
- [ ] Production Telegram MiniApp still works correctly with `src: telegram`
- [ ] No console errors in test mode or production
