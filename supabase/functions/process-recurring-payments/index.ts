...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch ALL tenant settings with Robokassa configured
    const { data: allSettings, error: settingsError } = await supabaseAdmin
      .from("admin_settings")
      .select("tenant_id, robokassa_merchant_login, robokassa_password1, robokassa_test_mode, telegram_bot_token, channel_name, grace_period_days, notification_payment_success, notification_payment_failed")
      .not("robokassa_merchant_login", "is", null)
      .not("robokassa_password1", "is", null);

    if (settingsError || !allSettings || allSettings.length === 0) {
      console.error("No tenants with Robokassa configured:", settingsError);
      return new Response(
        JSON.stringify({ error: "Robokassa not configured for any tenant" }),
    ...
    for (const settings of allSettings) {
      ...
      console.log(`[process-recurring] Processing tenant ${tenantId}`);
      ...
      // Query subscribers for THIS tenant only
      const { data: subscriptions, error: subError } = await supabaseAdmin
        ...
        .not("auto_renewal_consent_date", "is", null)
        .gte("subscription_end", oneDayFromNow.toISOString())
        .lte("subscription_end", threeDaysFromNow.toISOString());

      if (subError) {
        console.error(`[process-recurring] Error fetching subscriptions for tenant ${tenantId}:`, subError);
        continue;
      }

      if (!subscriptions || subscriptions.length === 0) {
        console.log(`[process-recurring] No subscriptions to process for tenant ${tenantId}`);
        continue;
      }

      console.log(`[process-recurring] Found ${subscriptions.length} subscriptions for tenant ${tenantId}`);
      ...
      for (const subscription of subscriptions) {
        try {
          ...
          if (paymentError) {
            ...
            allResults.push({ subscriber_id: subscription.id, success: false, message: "Failed to create payment" });
            continue;
          }
          ...
          if (responseText.toUpperCase().includes("OK")) {
            ...
            allResults.push({ subscriber_id: subscription.id, success: true, message: "Recurring payment completed" });
          } else {
            await supabaseAdmin
              .from("payment_history")
              .update({ 
            ...
            allResults.push({ subscriber_id: subscription.id, success: false, message: `Robokassa error: ${responseText}` });
          }
    ...
    console.log(`Processed ${allResults.length} subscriptions, ${successCount} successful`);
